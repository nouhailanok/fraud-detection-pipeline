...
class FraudLSTM(nn.Module):
    ...
    Note Opacus : DPLSTM ne supporte pas le dropout inter-couches natif
    (incompatible avec le calcul per-sample gradients).
    Le dropout est donc appliqué manuellement après le LSTM,
    avant le classifier — comportement équivalent en régularisation.
    """
    def __init__(
        self,
        ...
        if self.use_dplstm:
            # DPLSTM Opacus — pas de dropout inter-couches (per-sample gradient incompatible)
            self.lstm = DPLSTM(
            ...
            # LSTM standard PyTorch — dropout inter-couches natif disponible
            self.lstm = nn.LSTM(
                input_size=input_dim,
                hidden_size=hidden_dim,
                num_layers=num_layers,
        ...
        # Dropout explicite après LSTM — appliqué sur l'état caché final
        # Compense l'absence de dropout inter-couches dans DPLSTM
        self.post_lstm_dropout = nn.Dropout(dropout_rate)

        # Classifier MLP : hidden → 64 → 1
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Extraction du dernier état caché (dernière couche)
        # h_n shape : (num_layers, batch_size, hidden_dim)
        final_hidden = h_n[-1, :, :]   # (batch_size, hidden_dim)

        # Dropout après LSTM (regularisation — remplace dropout inter-couches)
        final_hidden = self.post_lstm_dropout(final_hidden)
    ...
    def info(self) -> str:
        ...
        return (
            ...
            f"Dropout    : {self.dropout_rate} (post-LSTM + classifier)\n"
