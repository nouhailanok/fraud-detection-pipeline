...
if ($Role -eq "Launcher") {
    ...
    Write-Output "Launching Federated Learning Pipeline with Adversarial Node..."
    ...
    $wtArgs = "new-tab --title `"Serveur`" powershell -NoExit -Command `"& '$ScriptPath' -Role Server`" `; " +
              ...
              "new-tab --title `"Client 4 (POISON)`" powershell -NoExit -Command `"& '$ScriptPath' -Role Client4`""
...
switch ($Role) {
    "Server" {
        ...
        $runBase   = "logs\runs"
        $runName   = "${timestamp}_$($env:NOM_USER)_pentest"
        ...
        # Round d'activation de la BA (Behavioral Analysis)
        $env:BA_ACTIVATION_ROUND = "5"  # Round où la behavioral analysis commence à détecter les clients malveillants
        # $env:BA_CONTAMINATION = "0.25"   # Proportion de clients malveillants (ex: 0.25 pour 1 sur 4)
    ...
    "Client4" {
        ...
        $env:ATTACK_MODE = "SIGN_FLIP"      # Can be changed to NOISE or SCALE
        $env:ATTACK_INTENSITY = "1.0"       # Adjust based on the mode chosen
        $env:ATTACK_START_ROUND = "3"       # The round where the corruption begins

        Write-Output "=== BANQUE 4 (MALICIOUS NODE) Python 3.11.9 RTX 4060 ==="
        # Note the path change here to execute the poison client
        & $PY federated/poisoning_client.py
