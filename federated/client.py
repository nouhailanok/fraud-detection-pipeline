...
from models.fraud_rnn import FraudRNN # On importe ton super modèle

# Détection automatique de ta RTX 3050 Ti
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Entraînement sur : {DEVICE}")

# --- 1. DÉFINITION DU CLIENT FLOWER ---
class FraudClient(fl.client.NumPyClient):
    def __init__(self, model, trainloader, testloader):
        self.model = model
        self.trainloader = trainloader
        self.testloader = testloader
        # On utilise notre fameuse fonction de perte optimisée
        self.criterion = nn.BCEWithLogitsLoss() 

    def get_parameters(self, config):
        """Extrait les poids du modèle PyTorch sous forme de liste NumPy pour l'envoi réseau."""
        return [val.cpu().numpy() for _, val in self.model.state_dict().items()]

    def set_parameters(self, parameters):
        """Injecte les poids reçus du serveur dans notre modèle local."""
        params_dict = zip(self.model.state_dict().keys(), parameters)
        state_dict = OrderedDict({k: torch.tensor(v) for k, v in params_dict})
        self.model.load_state_dict(state_dict, strict=True)

    def fit(self, parameters, config):
        """La phase d'entraînement local déclenchée par le serveur."""
        ...
        for epoch in range(1): # 1 epoch par round fédéré
...
=======
        ...
        with torch.no_grad():
            for X_batch, y_batch in self.testloader:
                ...
                # On transforme les logits bruts en prédictions binaires (0 ou 1)
...
if __name__ == "__main__":
    # 1. On instancie le modèle et on l'envoie sur la carte graphique
    model = FraudRNN().to(DEVICE)
    
    # 2. ICI, TU APPELLERAS LE DATALOADER DE TON COLLÈGUE
    # trainloader, testloader = load_data_from_npy("data/tensors/")
    trainloader, testloader = [], [] # (Bouchon en attendant ton collègue)
    
    # 3. On démarre le client et on le connecte au serveur central
    print("Connexion au serveur central...")
    fl.client.start_client(
        server_address="127.0.0.1:8080", 
        client=FraudClient(model, trainloader, testloader).to_client()
    )
