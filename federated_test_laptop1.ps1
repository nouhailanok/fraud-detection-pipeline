# federated_test_laptop1.ps1
# Node 1 - Se connecte au Desktop (serveur)
# IMPORTANT : remplacer SERVER_IP par l IP du Desktop (ipconfig sur Desktop)


$DIR = "$PSScriptRoot"  # Dossier du script actuel
$PY = "$DIR\venv\Scripts\python.exe"  # Python dans le venv du projet
...
Read-Host "Appuyer sur Entree"
"@ | Out-File -FilePath "$TMP\client1.ps1" -Encoding UTF8

Start-Process powershell -ArgumentList "-NoExit", "-ExecutionPolicy", "Bypass", "-File", "$TMP\client1.ps1"
Write-Output "Node 1 lance - connexion vers $SERVER_IP:8090"
