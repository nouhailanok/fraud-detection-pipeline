...
class FraudDetectionService:
    ...
    def _load_history_from_csv(self, csv_path: Path) -> None:
        ...
        for pan_id, group in df.groupby("cc_num"):
            ...
            for _, row in group.iterrows():
                try:
                    ...
                    if n_errors == 0:
                        print(f"  ❌ Première erreur pipeline : {type(e).__name__}: {e}")
                        print(f"     Colonnes disponibles : {list(row.index)}")
                        # Vérifier les colonnes critiques
                        for col in ["trans_date_trans_time", "amt", "cc_num", "lat", "long", "merchant", "dob"]:
                            val = row.get(col, "MANQUANT")
                            print(f"     {col:30s} = {val}")
                    n_errors += 1
                    continue
        ...
        known_history = len(self.user_history)
        known_enricher = len(enricher.client_state)
        print(f"     user_history  : {known_history} users ✅")
        print(f"     enricher cache: {known_enricher} users {'✅' if known_enricher > 0 else '❌ VIDE — features temporelles seront 0'}")
